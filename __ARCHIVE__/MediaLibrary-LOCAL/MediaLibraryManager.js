...
class MediaLibraryManager {
    ...
    renderModal() {
        console.log('[DEBUG] renderModal called');
        // Remove existing modal if any
        this.removeModal();

        // --- Overlay logic ---
        let overlay = document.querySelector('.media-library-overlay');
        if (!overlay) {
        ...
        // --- Always update modal content after modal creation to ensure grid and handlers are attached ---
    ...
    updateModalContent() {
        console.log('[DEBUG] updateModalContent called');
        ...
        if (this.currentTab === 'tvshows') {
            ...
            // For all other tabs (MOVIES, TV-SHOWS, COLLECTIONS, FAVORITES), use the existing renderMediaGrid logic
            this.renderMediaGrid();
        }
    }

    renderMediaGrid() {
        console.log('[DEBUG] renderMediaGrid called');
        const grid = document.getElementById('mediaGrid');
        if (!grid) return;
        ...
        items.forEach(item => {
            ...
            // Card click (open details, ignore action buttons)
            card.onclick = (e) => {
                if (
            ...
            // PosterSelector button handler
            const psBtn = card.querySelector('.poster-selector-btn');
            if (psBtn) {
                psBtn.onclick = (e) => {
                    console.log('[DEBUG] PosterSelector button clicked', item, window.PosterSelector);
                    e.preventDefault();
                    e.stopPropagation();
                    if (window.PosterSelector && item) {
                        ...
                        console.error('[PosterSelector] PosterSelector is not available or item not found.', { windowPosterSelector: window.PosterSelector, item, currentTab: this.currentTab });
                        this.showToast('PosterSelector is not available.');
                    }
                    console.log('[DEBUG] Handler attached to PosterSelector button', item, psBtn);
                };
            }
            ...
            card.querySelector('.collection-btn').onclick = (e) => {
                console.log('[DEBUG] Collection button clicked', item, window.PosterSelector);
                e.stopPropagation();
                if (window.PosterSelector && item) {
                    const mode = this.currentTab === 'tvshows' ? 'tv' : 'movie';
                    const selector = new window.PosterSelector(mode);
    ...
    showMovieDetailsModal(movie) {
        ...
        document.getElementById('detailsCollectionBtn').onclick = (e) => {
            ...
            // Open PosterSelector modal instead
            const cardElem = e.currentTarget.closest('.media-library-movie-card');
            const itemPath = cardElem ? cardElem.getAttribute('data-path') : '';
            let itemObj = null;
            if (this.currentTab === 'tvshows') {
                const tvShows = this.getTVShows();
                itemObj = tvShows.find(show =>
                    (show.path || '').replace(/\\/g, '/').toLowerCase().trim() === (itemPath || '').replace(/\\/g, '/').toLowerCase().trim()
                );
            } else {
                const items = this.getFilteredAndSortedItems();
                itemObj = items.find(i =>
                    (i.path || '').replace(/\\/g, '/').toLowerCase().trim() === (itemPath || '').replace(/\\/g, '/').toLowerCase().trim()
                );
            }
            if (window.PosterSelector && itemObj) {
                const mode = this.currentTab === 'tvshows' ? 'tv' : 'movie';
                const selector = new window.PosterSelector(mode);
                selector.getMediaContext = () => ({
                    mediaId: itemObj.path,
                    name: itemObj.name || itemObj.title,
                    path: itemObj.path,
                    type: mode
                });
                selector.onPosterSelected = ({filePath, posterType, poster}) => {
                    if (mode === 'movie' && this.moviePosters) {
                        this.moviePosters[itemObj.path] = filePath;
                        this.cacheBusters[itemObj.path] = Date.now();
                        this.renderMediaGrid();
                        this.showToast('Poster updated!');
                    } else if (mode === 'tv' && this.tvPosters) {
                        this.tvPosters[itemObj.name || itemObj.title] = filePath;
                        this.renderMediaGrid();
                        this.showToast('Poster updated!');
                    }
                };
                ...
                console.error('[PosterSelector] PosterSelector is not available or item not found.', { windowPosterSelector: window.PosterSelector, item: itemObj, itemPath, currentTab: this.currentTab });
                this.showToast('PosterSelector is not available.');
            }
        };
    }
