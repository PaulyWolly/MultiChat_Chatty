...
const backupRoutes = require('./routes/backups.routes.js');
...
// async function handleGPT4oMiniResponse(completion, res, originalMessage, startTime) {
//     let fullResponse = '';
//     const isRecipeRequest = originalMessage.toLowerCase().includes('recipe for') || originalMessage.toLowerCase().includes('how to make');

//     try {
//         // Step 1: Accumulate the full response from the AI
...
//         // Step 2: If it's a recipe, reformat the title
//         if (isRecipeRequest && fullResponse.includes('Here is a delicious recipe for')) {
//             const titleMatch = fullResponse.match(/^(.*?)(?=\s*Here is a delicious recipe for)/is);
//             if (titleMatch) {
//                 let title = titleMatch[1].trim();
//                 // Convert to Title Case
//                 const correctlyCasedTitle = title.toLowerCase().replace(/\b\w/g, char => char.toUpperCase());
//                 // Replace the all-caps title in the full response
//                 fullResponse = fullResponse.replace(title, correctlyCasedTitle);
//             }
//         }
        
//         // Step 3: Stream the corrected, full response to the client
//         // We send it chunk by chunk to simulate streaming for the client-side UI
...
//         console.log('Successfully streamed corrected response.');

//     } catch (error) {
//         console.error('Error in handleGPT4oMiniResponse:', error);
//         res.write(`data: ${JSON.stringify({ error: "Failed to handle AI response stream." })}\n\n`);
//     } finally {
...
async function connectWithRetry() {
    try {
        ...
        // Initialize systems that depend on DB connection
...
// =====================================================
// YOUTUBE API QUOTA MANAGEMENT (PERSISTENT)
// =====================================================
let dailyQuotaUsed = 0;
let quotaResetTime = new Date();
const pendingRequests = new Map(); // For deduplicating identical simultaneous requests

async function initializeQuota() {
    console.log('🔄 Initializing YouTube API Quota from database...');
    const storedQuota = await SettingsService.get('youtube_quota_used', 0);
    const storedResetTime = await SettingsService.get('youtube_quota_reset_time', new Date().toISOString());

    dailyQuotaUsed = parseInt(storedQuota, 10) || 0;
    quotaResetTime = new Date(storedResetTime);

    // If the reset time is in the past, reset the quota now
    if (new Date() >= quotaResetTime) {
        console.log('🗓️ Quota reset time has passed. Resetting now.');
        dailyQuotaUsed = 0;
        const newResetTime = new Date();
        newResetTime.setUTCHours(0, 0, 0, 0);
        newResetTime.setDate(newResetTime.getDate() + 1);
        quotaResetTime = newResetTime;
        await SettingsService.set('youtube_quota_used', dailyQuotaUsed);
        await SettingsService.set('youtube_quota_reset_time', quotaResetTime.toISOString());
    }
    
    console.log(`📊 Quota initialized: ${dailyQuotaUsed}/10000. Resets at ${quotaResetTime.toLocaleString()}`);
}
...
async function trackQuotaUsage(cost) {
...
async function getCachedOrFetch(cacheKey, fetchFunction, quotaCost) {
    ...
    if (remainingQuota < quotaCost) {
        throw new Error(`Quota exceeded: ${10000 - remainingQuota}/10000 calls used. Resets at ${quotaResetTime.toISOString()}`);
    }
...
app.post('/api/youtube/search', async (req, res) => {
    ...
    if (type === 'play') {
        ...
        try {
            ...
            const currentRemainingQuota = await getRemainingQuota();
            return res.json({
                ...result,
                quota: {
                    used: 10000 - currentRemainingQuota,
                    limit: 10000
                }
            });
        } catch (error) {
...
app.get('/api/youtube/quota-status', async (req, res) => {
    ...
    const quotaInfo = {
        ...
        remaining: remaining,
        total: 10000,
        percentage: Math.round(((10000 - remaining) / 10000) * 100),
