...
export default class YouTubeSearchManager {
    ...
    async handleYoutubeRequest(subject, isPagination = false, messageText = null, options = {}) {    
        ...
        // Detect channel search using switch statement
        let type = 'search';
        ...
        if (!isPagination) {
            ...
            console.log('🎯 [NEW-SEARCH] Adding to access history for dropdown:', subject);
            this.addToAccessHistory(subject);
        } else {
    ...
    extractQueryFromCacheKey(cacheKey) {
        try {
            ...
            if (cacheKey.startsWith('yt_search_') || cacheKey.startsWith('yt_channel_')) {
                const parts = cacheKey.split('_');
                // Format: ['yt', 'search'/'channel', 'query', 'p1', 'none']
     ...
     */
    addToAccessHistory(query) {
        const cleanQuery = this.cleanQueryForDisplay(query);
        const currentTime = Date.now();
        
        console.log('📈 [ACCESS-HISTORY] Adding query to access history:', cleanQuery);
    ...
    async renderQueryDropdown(queries, currentQuery) {
        ...
        queries.slice(0, 20).forEach((query, index) => {
            ...
            // Store both the original full query and clean display name
            const cleanDisplayName = this.cleanQueryForDisplay(query.query);
            item.setAttribute('data-query', query.query); // Full original query
            item.setAttribute('data-display-name', cleanDisplayName); // Clean display name
            ...
            const handleQueryClick = async (e) => {
                ...
                console.log('🖱️ [CLICK] Clean display name:', cleanDisplayName);
            ...
            queryName.textContent = cleanDisplayName;
            queryName.title = `Run search for: "${cleanDisplayName}"`;
    ...
    async navigateToQuery(query) {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key && key.startsWith('yt_') && (key.includes('_search_') || key.includes('_channel_'))) {
                const extractedQuery = this.extractQueryFromCacheKey(key);
    ...
    createHoverCloseButton(container) {
        ...
        controlOverlay.style.cssText = `
            ...
            background: linear-gradient(135deg, transparent 0%, rgba(0,0,0,0.1) 30%, rgba(0,0,0,0.7) 100%) !important;
        ...
        // Add hover effects to the close button - YouTube style with red icon
        hoverCloseBtn.addEventListener('mouseenter', () => {
            hoverCloseBtn.style.background = 'rgba(255, 255, 255, 0.1) !important';
            iconElement.style.color = '#ff2222 !important'; // Brighter red on hover
            labelElement.style.color = '#ff4444 !important';
            labelElement.style.opacity = '1 !important';
        });

        hoverCloseBtn.addEventListener('mouseleave', () => {
            hoverCloseBtn.style.background = 'transparent !important';
            iconElement.style.color = '#ff4444 !important'; // Red by default
            labelElement.style.color = 'white !important';
            labelElement.style.opacity = '0.9 !important';
        });
