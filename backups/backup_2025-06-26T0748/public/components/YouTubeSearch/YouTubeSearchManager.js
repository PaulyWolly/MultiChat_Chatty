...
export default class YouTubeSearchManager {
                ...
                if (isOldFormat || isNewFormat || isNewChannelFormat) {
                    try {
                        ...
                        if (query) {
                            ...
                            if (existingQuery) {
                                // Update existing query with cache info
    ...
    getCachedPageCount(query) {
        ...
        while (page <= 50) { // Check up to 50 pages max
            ...
            const possibleKeys = [
                // NEW FORMAT (matches handleYoutubeRequest exactly): yt_search_avocado_tree_p1_none
    ...
    showVideo(videoId) {
        // Create a beautiful YouTube Link Modal instead of embedded player
        console.log('🎬 [VIDEO-MODAL] Creating YouTube link modal for video:', videoId);
        
        this.createVideoContainer();
        const videoWrapper = document.getElementById('youtube-video');
        
        if (!videoWrapper) {
            console.error('❌ [VIDEO-MODAL] Video wrapper not found! Cannot show modal.');
            return;
        }
        
        // Get video data for a rich modal experience
        const videoData = this.findVideoData(videoId);
        console.log('🎬 [VIDEO-MODAL] Video data found:', !!videoData);
        
        // Create clean auto-playing video modal (no unnecessary headers!)
        videoWrapper.innerHTML = `
            <div class="youtube-link-modal">
                <div class="modal-content">
                    <div class="video-player">
                        <iframe 
                            ...
                            allowfullscreen
                            style="width: 100%; height: 75vh; border-radius: 12px; box-shadow: 0 8px 32px rgba(0,0,0,0.6);">
                        </iframe>
                    </div>
                    ...
                    <div class="video-info">
                        ...
                        ${videoData?.duration ? `
                            <p class="video-duration">
                                <strong>⏱️ Duration:</strong> ${videoData.duration}
                            </p>
                        ` : ''}
                        
                        ${videoData?.description ? `
                            <p class="video-description">
                                ${videoData.description.substring(0, 250)}${videoData.description.length > 250 ? '...' : ''}
                            </p>
                        ` : ''}
                    </div>
                </div>
        ...
        if (typeof updateStatus === 'function') {
            updateStatus('🎬 Auto-Playing YouTube Video');
        }
        
        console.log('✅ [VIDEO-MODAL] Auto-playing YouTube video modal created successfully');
    }
