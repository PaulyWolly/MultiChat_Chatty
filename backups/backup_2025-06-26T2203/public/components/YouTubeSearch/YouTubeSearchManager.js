...
export default class YouTubeSearchManager {
    ...
    /**
     * Scroll to TOP of page for YouTube results - ALWAYS scroll to top!
     */
    scrollToYouTubeResults() {
        console.log('📍 [SCROLL] ALWAYS scrolling to TOP for YouTube results');
        try {
            // ALWAYS scroll to the very top of the page for YouTube searches
            window.scrollTo({
                top: 0,
                left: 0,
                behavior: 'smooth'
            });
            console.log('📍 [SCROLL] Successfully scrolled to TOP of page');
        } catch (error) {
     ...
     */
    forceScrollToTop() {
        console.log('📍 [FORCE-SCROLL] Forcing immediate scroll to TOP');
        try {
            window.scrollTo(0, 0);
            // Also scroll the document element and body to ensure compatibility
            document.documentElement.scrollTop = 0;
            document.body.scrollTop = 0;
            console.log('📍 [FORCE-SCROLL] Immediate scroll to top completed');
        } catch (error) {
    ...
    showMinimizedPaginatorBar() {
        ...
        if (!minimizedBar) {
            ...
            minimizedBar.onclick = () => {
                console.log('🔄 [PAG-SWITCH] Minimized paginator clicked - scrolling to TOP first');
                // CRITICAL: Always scroll to top when switching back to YouTube search
                this.forceScrollToTop();
                // Small delay to ensure scroll completes before showing paginator
                setTimeout(() => {
                    this.setPaginatorBar('restored');
                    // Double-check scroll after restoration
                    setTimeout(() => {
                        this.scrollToYouTubeResults();
                    }, 100);
                }, 50);
    ...
    renderRestoredPaginatorBar(page, totalPages, subject) {
        ...
        restoredBar.innerHTML = `
            <div class="pagination-drag-tab">
                ...
                <div>
                    <div >
                       <img class="youtube-legend" src="/assets/img/youtube-legend1.png" alt="YouTube Legend" class="youtube-legend-icon">
                    </div>
                </div>
