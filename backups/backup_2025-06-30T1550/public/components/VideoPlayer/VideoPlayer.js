class VideoPlayer {
    ...
    createPlayer() {
        ...
        try {
            if (typeof window.videojs === 'undefined') {
                console.error('🎬 [VIDEO-PLAYER] Video.js library not loaded');
                this.vjsPlayer = null;
                return;
            }
    ...
    createControls() {
        ...
        this.watchLaterButton.onclick = () => {
            // Use currentMediaItem if available, else fallback to currentFile
            let movie = this.currentMediaItem || this.currentFile;
            // Try to get current time and duration from Video.js if available
            ...
            if (window.mediaLibraryManager && typeof window.mediaLibraryManager.saveResumeProgress === 'function') {
                ...
                if (window.mediaLibraryManager.showToast) {
                    window.mediaLibraryManager.showToast('Saved to Watch Later!');
                }
            }
        };
    ...
    togglePlay() {
        if (!this.vjsPlayer) {
            console.warn('🎬 [VIDEO-PLAYER] Video.js player not initialized');
            return;
        }
    ...
    updateProgress() {
        if (!this.vjsPlayer || !this.vjsPlayer.duration()) return;
